...
export function PlayingCard({ rank, suit, size = 'md', faceDown = false, className }: PlayingCardProps) {
  ...
  if (faceDown) {
    return (
      <div
        className={cn(
          'rounded-lg border-2 border-slate-600',
...
export function generateCardsFromHand(hand: string): Array<{ rank: string; suit: 's' | 'h' | 'd' | 'c' }> {
